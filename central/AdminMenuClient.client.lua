...
local function createAdminInterface()
	...
	local function renderAwaken()
		...
		-- (V11) HISTÓRIA. O card do Despertar mostra imagem, nome, história
		-- e as Tools; sem este campo a história ficava sempre vazia, porque
		-- o payload é montado explicitamente logo abaixo.
		label(catalogContent, "HISTÓRIA:", UDim2.new(0.3, 0, 0.055, 0), UDim2.new(0.02, 0, 0.48, 0))
		local loreBox = textBox(catalogContent, "Aparece no card do Despertar", UDim2.new(0.64, 0, 0.055, 0), UDim2.new(0.34, 0, 0.48, 0), awakenDraft.lore)
		loreBox:GetPropertyChangedSignal("Text"):Connect(function()
