-- ============================================
-- DATA MANAGER SERVER V6 — ZERO PERSONAGENS HARDCODED
-- Coloque em ServerScriptService
-- Nome: "DataManager"
-- SUBSTITUI: DataManager_Server_Script_V5
-- REMOVER:   DataManager_Server_Script_V5
-- ============================================
-- (V6) ALTERAÇÕES:
...
local function validateData(data)
	...
	if data.ownedCharacters then
		if isOldFormat(data.ownedCharacters) then
			print("[DATA V6] Migrando ownedCharacters de V3 (strings) para V4 (objetos)...")
			validated.ownedCharacters = migrateCharacterList(data.ownedCharacters)
			print("[DATA V6] Migração concluída: " .. #validated.ownedCharacters .. " personagens")
		else
			...
			for _, charObj in ipairs(data.ownedCharacters) do
				if type(charObj) == "table" and type(charObj.name) == "string" then
					...
					table.insert(validated.ownedCharacters, {
...
local function loadPlayerData(player)
	...
	while attempts < maxAttempts do
		...
		if success then
			...
			if result and isOldFormat(result.ownedCharacters) then
				print("[DATA V6] Salvando dados migrados de " .. player.Name)
				task.spawn(function()
					local ok, err = pcall(function()
						playerDataStore:SetAsync(userId, data)
					end)
					if ok then
						print("[DATA V6] Dados migrados salvos: " .. player.Name)
					else
						warn("[DATA V6] Erro ao salvar migração: " .. tostring(err))
					end
				end)
			end

			print(
				"[DATA V6] Dados carregados: "
					.. player.Name
					.. " ("
			...
			warn(
				"[DATA V6] Erro ao carregar dados de "
					.. player.Name
					.. " (tentativa "
					.. attempts
					.. "): "
	...
	warn("[DATA V6] Usando dados padrão para " .. player.Name)
...
local function savePlayerData(player, isLeaving)
	...
	while attempts < maxAttempts do
		...
		if success then
			print("[DATA V6] Dados salvos: " .. player.Name)
			isSaving[player] = false
			return true
		else
			warn(
				"[DATA V6] Erro ao salvar "
					.. player.Name
					.. " (tentativa "
					.. attempts
					.. "): "
...
task.spawn(function()
	while true do
		...
		if #players > 0 then
			print("[DATA V6] Auto-save: " .. #players .. " jogadores")
		end
	end
end)
...
game:BindToClose(function()
	print("[DATA V6] Servidor fechando, salvando todos os dados...")
	local players = Players:GetPlayers()
	...
	print("[DATA V6] Shutdown save completo")
end)
...
print([[
╔══════════════════════════════════════════════════════╗
║  DATA MANAGER SERVER V6 — ZERO PERSONAGENS          ║
╠══════════════════════════════════════════════════════╣
║  SUBSTITUI: DataManager_Server_Script_V5             ║
║  REMOVER:   DataManager_Server_Script_V5             ║
╠══════════════════════════════════════════════════════╣
║  * Inventário novo começa VAZIO (sem Noob forçado)   ║
║  * MIGRATION_CONFIG zerado (sem Faker Gui/etc)       ║
║  * removeCharacterByName sem trava de nome           ║
║    (necessário pra remoção via catálogo V4)          ║
║  * DATASTORE_NAME/DATA_VERSION intactos — saves OK   ║
║  * claimedAchievements agora preservado no validate  ║
╚══════════════════════════════════════════════════════╝
]])
